file_path = "/kaggle/working/src/training/experiments/experiment5.py"
...
dataset:
  ...
  batch_size: 256
  num_workers: 0
  data_dir: "/kaggle/input/fii-atnn-2025-project-noisy-cifar-100/fii-atnn-2024-project-noisy-cifar-100"
  mean: [0.485, 0.456, 0.406]
  std: [0.229, 0.224, 0.225]
  resize_resolution: 260  # Resize resolution for images (can be single value or [width, height])

model:
  name: "tf_efficientnet_b2_ns"       # MLP, ResNet18, ResNet50, ResNest14d, ResNest26d
...
augmentation:
  ...
  mixup_alpha: 0.0  # MixUp alpha parameter (0.0 = disabled, typical: 0.5-1.0)
...
cache:
  ram_limit_gb: 18  # RAM limit in GB for hybrid caching (items beyond this will be cached to SSD)
  ssd_path: "/kaggle/input/ssd-tensors/temp"  # Path to SSD cache directory for hybrid caching
...
sweep:
  ...
  parameters:
    ...
    T_0:
      values: [390]  # Number of iterations for the first restart (~0.5, 1, 2 epochs for batch_size=256)

    T_mult:
      values: [1]  # Factor that increases T_i after a restart

    eta_min:
      values: [1e-6]  # Minimum learning rate

    randaugment_magnitude:
      values: [5]  # Magnitude of RandAugment operations (typical: 5-15)
